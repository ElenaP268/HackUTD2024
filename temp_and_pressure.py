...
df = pd.read_excel('Resolute_728H-10_14-10_21_pressure - Copy.xlsx', index_col=None)  # Replace with your actual CSV file path
...
df['Pressure (MPa)'] = df['Pressure (MPa)'].fillna(method='ffill')
...
df = pd.read_csv('updated_file_numeric_time.csv', index_col=False)  # Replace with your actual CSV file path

# Create a scatter plot between 'time' and another variable, e.g., 'B'
plt.figure(figsize=(8, 6))  # Optional: Set the size of the plot
plt.scatter(df['numeric_time'], df['Pressure (MPa)'], color='blue', label='Pressure vs Time')

# Add labels and title
plt.xlabel('Time (seconds)')
plt.ylabel('Pressure (MPa)')
plt.title('Scatter plot of Time vs Pressure')
...
y = df['Pressure (MPa)']    # Dependent variable
...
# Visualize the linear regression line on the scatter plot
plt.scatter(df['numeric_time'], df['Pressure (MPa)'], color='blue', label='Data')
...
# plt.scatter(outlier_X, outlier_y_actual, color='orange', label='Outliers')
# plt.scatter(df['numeric_time'], df['Pressure (MPa)'], color='blue', label='Data')
# plt.plot(df['numeric_time'], model.predict(df[['numeric_time']]), color='red', label='Regression Line')
...
# updated pressure df
new_df_pressure = df[['numeric_time', 'Pressure (MPa)']]
new_df_pressure.to_csv('new_pressure_file.csv', index=False)


df = pd.read_excel('Resolute_728H-10_14-10_21_temperature - Copy.xlsx', index_col=None)  # Replace with your actual CSV file path
...
df['Inj Gas Meter Volume Setpoint'] = df['Inj Gas Meter Volume Setpoint'].fillna(method='ffill')
df['Inj Gas Meter Volume Instantaneous'] = df['Inj Gas Meter Volume Instantaneous'].fillna(method='ffill')
df['Temperature (C)'] = df['Temperature (C)'].fillna(method='ffill')
...
# Create a scatter plot between 'time' and another variable, e.g., 'B'
plt.figure(figsize=(8, 6))  # Optional: Set the size of the plot
plt.scatter(df['numeric_time'], df['Temperature (C)'], color='blue', label='Temperature vs Time')

# Add labels and title
plt.xlabel('Time (seconds)')
plt.ylabel('Temperature (C)')
plt.title('Scatter plot of Time vs Temperature')
...
# Prepare the data
X = df[['numeric_time']]  # Independent variable (must be in 2D array)
y = df['Temperature (C)']    # Dependent variable
...
# Visualize the linear regression line on the scatter plot
plt.scatter(df['numeric_time'], df['Temperature (C)'], color='blue', label='Data')
plt.plot(df['numeric_time'], model.predict(df[['numeric_time']]), color='red', label='Regression Line')
plt.title('Linear Regression: x_column vs y_column')
plt.xlabel('x_column')
plt.ylabel('y_column')
...
# updated temp df
new_df_temp = df[['numeric_time', 'Temperature (C)']]
new_df_temp.to_csv('new_temp_file.csv', index=False)
